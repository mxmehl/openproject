...
class AccountController < ApplicationController
  ...
  def change_password
    ...
    change_password_flow(user: @user, params: params) do
      password_authentication(params[:username], params[:new_password])
    end
  end
    ...
    if user.nil?
      ...
      if user and user.check_password?(password)
        ...
        if not user.active?
          ...
          render_password_change(user, I18n.t(:notice_account_new_password_forced))
