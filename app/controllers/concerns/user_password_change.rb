...
module Concerns::UserPasswordChange
  ...
  def change_password_flow(user:, params:, check_password: true, update_legacy: true)
    ...
    if check_password && user.check_password?(params[:password], update_legacy: update_legacy) != true
      flash_and_log_invalid_credentials
      return render_password_change user, I18n.t(:notice_account_wrong_password)
    end
  ...
  def render_password_change(user, message)
    flash[:error] = message
    @user = user
